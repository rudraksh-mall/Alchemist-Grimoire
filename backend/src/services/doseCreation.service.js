...
export const createInitialDoses = async (schedule) => {
  ...
  //  Normalize dates to local midnight for stable calendar day comparison
  //  When dealing with user input, it's safest to define the "calendar day"
  ...
  if (dosesToCreate.length > 0) {
    try {
      ...
      if (error.code !== 11000) {
        ...
        console.warn(
          "[Circus Crier] Duplicate dose insert attempted and ignored."
        );
      }
    }
  }
