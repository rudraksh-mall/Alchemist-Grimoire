...
export function RemindersPage() {
  ...
  const handleTakeDose = async (id) => {
    try {
      ...
      toast.success(
        "Potion consumed! The mystical energies flow through you. ✨"
      );
    } catch (error) {
  ...
  const handleSkipDose = async (id) => {
    try {
      ...
      toast.warning(
        "Dose skipped. The spirits understand, but consistency brings power. 🌙"
      );
    } catch (error) {
  ...
  const handleSnooze = async (id) => {
    toast.info(
      "Dose snoozed for 15 minutes. The crystal ball will remind you again. ⏰"
    );
  };
  ...
  const getStatusCount = (status) =>
    doses.filter((d) => d.status === status).length;
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {["pending", "taken", "missed", "skipped", "total"].map(
              (status, index) => (
                <motion.div
                  key={status}
                  initial={{ opacity: 0, y: 20 }}
