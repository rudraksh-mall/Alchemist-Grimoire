...
export function ScheduleFormPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(isEdit ? 'Failed to update potion' : 'Failed to add potion');
    } finally {
      setIsLoading(false);
    }
  };
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-muted-foreground mt-1">
                  {isEdit ? 'Modify your mystical medicine' : 'Create a new wellness potion for your grimoire'}
                </p>
              </div>
            </div>
          </motion.div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="border border-border rounded-lg p-4 bg-muted/20">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-medium">{formData.name || 'Potion Name'}</p>
                        <p className="text-sm text-muted-foreground">
                          {formData.dosage || 'Dosage'} • {formData.frequency || 'Frequency'}
                        </p>
                        {formData.times.length > 0 && formData.times[0] && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {formData.times.filter(t => t).map((time, index) => (
                              <Badge key={index} variant="secondary" className="text-xs">{time}</Badge>
                            ))}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex space-x-3 pt-4 border-t border-border">
                    ...
                    <Button type="submit" disabled={isLoading} className="flex-1 magical-glow">
                      {isLoading ? (
